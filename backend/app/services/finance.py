...
def record_manual_payment(
    ...
    order = db.scalar(
        select(Order).where(Order.id == payload.order_id, Order.store_id == store_id)
    )
    if not order:
    ...
    if payload.method == PaymentMethod.cash:
        ...
        raise ValueError(
            "Split payment cash_amount and upi_amount must be positive and equal amount"
        )
    ...
    ref = payload.provider_ref or (
        f"cash-{store_id}-{order.id}-{int(datetime.now(timezone.utc).timestamp() * 1000)}"
    )
    if db.scalar(
        select(Payment).where(Payment.store_id == store_id, Payment.provider_ref == ref)
    ):
...
def order_payment_summary(db: Session, store_id: int, order_id: int) -> dict:
    ...
    payments = list(
        db.scalars(
            select(Payment)
            .where(Payment.store_id == store_id, Payment.order_id == order_id)
    ...
    paid = round(
        sum(
            ...
            if payment.status not in {PaymentStatus.failed, PaymentStatus.duplicate}
        ),
        2,
    )
    refunded = round(sum(payment.refunded_amount for payment in payments), 2)
...
def request_refund(
    ...
    payment = db.scalar(
        select(Payment).where(Payment.id == payload.payment_id, Payment.store_id == store_id)
    )
    if not payment:
    ...
    record_audit_event(
        ...
        entity_type="refund",
        entity_id=f"payment:{payment.id}",
        actor_type="operator",
        actor_id=_actor(operator),
        evidence={"amount": payload.amount, "reason": payload.reason},
    )
    db.commit()
    db.refresh(refund)
    return refund
...
def decide_refund(
    ...
    refund = db.scalar(
        select(Refund).where(Refund.id == refund_id, Refund.store_id == store_id)
    )
    if not refund:
    ...
    refund.decided_by = _actor(operator)
    ...
    if notes:
        refund.reason = f"{refund.reason} | Decision: {notes}"
...
def cancel_order_financially(
    ...
    if order.status == OrderStatus.delivered:
        raise ValueError("Delivered orders require refund handling, not cancellation")
    ...
    record_audit_event(
        db,
        store_id=store_id,
        action=AuditAction.order_cancelled_financially,
        entity_type="order",
        entity_id=order.id,
        actor_type="operator",
        actor_id=_actor(operator),
...
def generate_settlement(
    ...
    payments = list(
        db.scalars(
            select(Payment).where(
                ...
                Payment.received_at.between(start, end),
                Payment.status.notin_([PaymentStatus.failed, PaymentStatus.duplicate]),
            )
        ).all()
    )
    cash = round(sum(payment.cash_amount for payment in payments), 2)
    upi = round(sum(payment.upi_amount for payment in payments), 2)
    refunds = float(
        db.scalar(
            select(func.coalesce(func.sum(Refund.amount), 0.0)).where(
                Refund.store_id == store_id,
                Refund.status == RefundStatus.approved,
                Refund.decided_at.between(start, end),
            )
        )
        or 0.0
    )
    ...
    settlement = db.scalar(
        select(DailySettlement).where(
            ...
            DailySettlement.business_day == business_day,
        )
    )
    if settlement and settlement.status == SettlementStatus.closed:
        return settlement
    if not settlement:
        settlement = DailySettlement(store_id=store_id, business_day=business_day)
        db.add(settlement)

    settlement.cash_total = cash
    settlement.upi_total = upi
    settlement.refund_total = round(refunds, 2)
    settlement.net_total = round(cash + upi - refunds, 2)
    ...
    record_audit_event(
        db,
        store_id=store_id,
        action=AuditAction.settlement_generated,
        entity_type="daily_settlement",
        entity_id=business_day,
        actor_type="operator",
        actor_id=_actor(operator),
...
def close_settlement(
    ...
    settlement = db.scalar(
        select(DailySettlement).where(
            DailySettlement.id == settlement_id,
            DailySettlement.store_id == store_id,
        )
    )
    if not settlement:
    ...
    settlement.closed_by = _actor(operator)
    if notes:
        settlement.notes = notes
    ...
    record_audit_event(
        ...
        evidence={
            ...
            "business_day": settlement.business_day,
        },
    )
    db.commit()
    db.refresh(settlement)
...
def list_settlements(db: Session, store_id: int) -> list[DailySettlement]:
    return list(
        db.scalars(
            ...
            .where(DailySettlement.store_id == store_id)
            .order_by(DailySettlement.business_day.desc())
        ).all()
    )
...
def accounting_rows(db: Session, store_id: int, day: str | None) -> list[dict]:
    ...
    return [
        {
            "date": payment.received_at.isoformat(),
            "type": "payment",
            ...
            "net_amount": round(payment.amount - payment.refunded_amount, 2),
...
def export_accounting(
    ...
    rows = accounting_rows(db, store_id, day)
    fields = [
        "date",
        "type",
        "reference",
